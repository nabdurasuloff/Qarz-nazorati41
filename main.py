...
class MibBazaTab(ttk.Frame):
    ...
    def toxtatish_yakunlash(self):
        ...
        self.wait_window(dlg)
        if dlg.result:
            sabab, sana = dlg.result
            db.mark_mib_yakunlandi(xat['id'], sabab, sana)
            messagebox.showinfo(
                "Saqlandi",
                "MIB ijro ishi to'xtatilgan/yakunlangan deb belgilandi. Endi bu mijoz "
                "'MIBda jarayondagi hujjatlar' ro'yxatidan chiqarilib, 'Yakunlangan "
                "ishlar' bo'limida ko'rinadi."
            )
            self.refresh()
...
class DavoArizaTab(ttk.Frame):
    def __init__(self, parent, app):
        ...
        cols = ('check', 'anketa', 'mijoz', 'turi', 'jami_qarz', 'yuborilgan', 'davo_holati',
        ...
        widths = {'check': 34, 'anketa': 85, 'mijoz': 160, 'turi': 60, 'jami_qarz': 90,
                  'yuborilgan': 85, 'davo_holati': 120, 'ish_raqami': 80, 'taminot': 50,
                  'tavsiya': 150, 'farq_holati': 140}
        for c in cols:
            self.tree.heading(c, text=headings[c])
            self.tree.column(c, width=widths[c], anchor='center' if c == 'check' else 'w')
        self.tree.tag_configure('otgan', foreground=ERR)
